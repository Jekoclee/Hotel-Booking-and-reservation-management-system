...
if (isset($_POST['getall_rooms'])) {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        if ($row['status'] == 1) {
            $status = "<button onclick='toggle_status($row[id],0)' class='btn btn-sm btn-success'>Active</button>";
        } else {
            $status = "<button onclick='toggle_status($row[id],1)' class='btn btn-sm btn-danger'>Inactive</button>";
        }
        ...
        <tr class='align-middle'>
            ...
            <td>₱$row[price]</td>
            <td>
            <span class='badge bg-success'>$row[adult] Adult</span> <br>
            <span class='badge bg-info'>$row[children] Children</span>
            </td>
            <td>$row[quantity]</td>
            <td>$status</td>
            <td>
               <button type='button' onclick='edit_bar($row[id])' class='btn btn-dark shadow-none btn-sm' data-bs-toggle='modal' data-bs-target='#edit-room'>
...
if (isset($_POST['edit_room'])) {
    ...
    $values = [$frm_data['name'], $frm_data['area'], $frm_data['price'], $frm_data['quantity'], $frm_data['adult'], $frm_data['child'], $frm_data['desc'], $frm_data['room_id']];
...
if (isset($_POST['get_room_images'])) {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        if ($row['thumb'] == 1) {
            ...
            $thumb_btn = "<button onclick='thumb_image($row[sr_no], $row[room_id])' class='btn btn-secondary'><i class='bi-check-lg'></i></button>";
        }
        echo <<<data
        <tr class='align-middle'>
               <td>
               ...
               <td> $thumb_btn</td>
               <td>
                <button onclick='rem_image($row[sr_no], $row[room_id])' class='btn btn-sm btn-danger'><i class='bi bi-trash'></i></button>
               </td>
        </tr>
        data;
    }
